...
<div class="row">
	...
	<tr>
	 ...
	 <div id="div_center" >
	 <table class="table" >
	 	<c:if test="${!empty my_club}">
	 	<thead  align="center">
		<tr>
			<th colspan="4"><font size="4"><b>내 동아리</b></font></th>
		</tr>
		<tr>
			<th width="20%" ><b>동아리명</b></th>
			<th width="80%" colspan="3">${my_club.club_name}</th>
		...
		<tr>
			<td width="10%"><b>동아리장</b></td>
			<td width="40%">${my_club_king}</td>
			<td width="25%"><b>동아리 인원수</b></td>
			<td width="25%"><b>${my_club_member}</b>명</td>
		</tr>
		<tr>
			<td><b>동아리 멤버</b></td>
			<td colspan="3">
			...
			<c:if test="${x.id eq sessionScope.userid}">
				<c:set var="manager" value="${x.club_flag}"></c:set>
			</c:if>
			<li><b>${x.name}</b>&nbsp;(${x.id})</li>
		...
		<tr>
			<td><b>동아리 소개</b></td>
			<td colspan="3">${my_club.club_introduce_text}</td>	
		</tr>
		<c:if test="${my_club_king eq sessionScope.userid or manager == 2}">
		<tr>
			<td colspan="4" align="center">
				<button class="btn btn-default" onclick="location.href='club_management.do'">동아리 관리</button>
			</td>
		</tr>
		</c:if>
		<c:if test="${my_club_king ne sessionScope.userid and manager != 2}">
		<tr>
			<td colspan="4" align="center">
				<button class="btn btn-default" onclick="location.href='club_secession.do?club_name=${my_club.club_name}'">탈퇴하기</button>
			</td>
		</tr>
		</c:if>
		</tbody>
		...
		<c:if test="${empty my_club}">
		 <thead>
		 	<tr>
		 		<td><font size="6"><b>가입된 동아리가 없습니다.</b></font> </td>
		 	</tr>
		 </thead>
		</c:if>
