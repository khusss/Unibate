...
<div class="row">
	...
	<tr>
	 ...
	 <thead>
	 <tr>
	 	<td colspan="3" align="center"><font size="4">동아리 관리</font></td>
	 </tr>
	 <tr>
	 	<td colspan="3" align="center"><font size="3">${my_club.club_name}</font></td>
	 </tr>
	 </thead>
	 <tbody>
	 <c:if test="${my_club_king eq sessionScope.userid}">
	 <c:forEach items="${my_club_member_list}" var="x" >	
	 	<c:if test="${x.id ne sessionScope.userid}">		
	 	<tr>
	 		<td align="center"><b>${x.name}</b>&nbsp;(${x.id})</td>
	 		<c:choose>
	 		<c:when test="${x.club_flag==3}">
	 			<td align="center">클럽원&nbsp;&nbsp;&nbsp;<button class="btn btn-default" onclick="location.href='manager_appointment.do?id=${x.id}&flag=2'">관리자로 임명</button></td>
	 		</c:when>
	 		<c:otherwise>
	 			<td align="center">관리자&nbsp;&nbsp;&nbsp;<button class="btn btn-default" onclick="location.href='manager_appointment.do?id=${x.id}&flag=3'">클럽원으로 강등</button></td>
	 		</c:otherwise>
	 		</c:choose>
	 		<td align="center"><button class="btn btn-default" onclick="location.href='club_fire.do?id=${x.id}&club_name=${my_club.club_name}'">강퇴시키기</button></td>
	 	</tr>
	 	</c:if>
	 	<c:if test="${x.id eq sessionScope.userid}">	
	 		<td colspan="3" align="center"><b>${x.name}</b>&nbsp;(${x.id}는 클럽장 입니다.)</td>
	 	</c:if>
	 	</c:forEach>
	 	</c:if>
	 	<c:if test="${my_club_king ne sessionScope.userid}">
	 	<c:forEach items="${my_club_member_list}" var="x" >	
	 		<c:if test="${x.id ne sessionScope.userid}">		
	 		<tr>
	 			<td align="center"><b>${x.name}</b>&nbsp;(${x.id})</td>
