			...
			<div class="container">
				<div class="row">
					<div class="col-lg-12">
						<div class="big-cta">
							<div class="cta-text">
								<h2><span><font size="20">동아리</font></span> 모집</h2>
							</div>
						</div>
						<div class="clearfix">
						</div>
								...
								<ul id="thumbs" class="portfolio">
									<div class="container">
										<table class="table">
											...
											<c:set var="number" value="${allRecruit-(pageNum-1)*15}" />
											<tbody>
											<c:forEach items="${rBoard}" var="rb">
												<tr>
													<td>${number}</td>
													<th scope="row"><a href="recruit_View.do?pageNum=${pageNum}&recruit_num=${rb.recruit_board_num}">${rb.recruit_subject}</a></th>
													<td>${rb.id}</td>
													<td>${rb.recruit_date}</td>
													<td>${rb.recruit_hits}</td>
												</tr>
										...
										<div class="text-center">
											<ul class="pagination pagination-large">
												...
												<c:if test="${pageNum > 10}">
													<li><a href="recruiting.do?pageNum=${pageNum-10}">Prev</a></li>
												</c:if>
												<c:if test="${pageBlock*10<pageCount}">
													<c:forEach var="i" begin="${pageStart}" step="1" end="${pageEnd}">
														<li><a href="recruiting.do?pageNum=${i}">${i}</a></li>
													</c:forEach>
												</c:if>
												<c:if test="${pageBlock*10>=pageCount}">
												...
												<c:if test="${pageCount >= pageNum+10 and pageCount > 10}">
													<li><a href="recruiting.do?pageNum=${pageNum+10}">Next</a></li>
												</c:if>
												<c:if test="${pageCount < pageNum+10 and pageCount > 10}">
													<li><a href="recruiting.do?pageNum=${pageCount}">Next</a></li>
												</c:if>
												<li>&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-lg" onclick="location.href='recruit_write.do?pageNum=${pageNum}'">
														글쓰기
													</button></li>
											</ul>
										</div>
