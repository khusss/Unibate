			...
			<div class="container">
				<div class="row">
					<div class="col-lg-12">
						<div class="big-cta">
							<div class="cta-text">
								<h2><span><font size="20">토론</font></span>주제건의</h2>
							</div>
						</div>
						<div class="clearfix">
						</div>
								...
								<ul id="thumbs" class="portfolio">
									<div class="container">
										<table class="table">
											...
											<tbody>
											<c:set var="number" value="${allSuggestion-(pageNum-1)*15}" />
											<c:forEach items="${sboard}" var="sb">
												<tr>
													<td>${number}</td>
													<th scope="row"><a href="suggestionView.do?pageNum=${pageNum}&suggest_num=${sb.suggestion_board_num}">${sb.suggestion_subject }</a></th>
													<td>${sb.id}</td>
													<td>${sb.suggestion_date}</td>
													<td>${sb.suggestion_hit}</td>
												</tr>
										...
										<div class="text-center">
											<ul class="pagination pagination-large">
												...
												<c:if test="${pageNum > 10}">
													<li><a href="suggestion.do?pageNum=${pageNum-10}">Prev</a></li>
												</c:if>
												<c:if test="${pageBlock*10<pageCount}">
													<c:forEach var="i" begin="${pageStart}" step="1" end="${pageEnd}">
														<li><a href="suggestion.do?pageNum=${i}">${i}</a></li>
													</c:forEach>
												</c:if>
												<c:if test="${pageBlock*10>=pageCount}">
												...
												<c:if test="${pageCount >= pageNum+10 and pageCount > 10}">
													<li><a href="suggestion.do?pageNum=${pageNum+10}">Next</a></li>
												</c:if>
												<c:if test="${pageCount < pageNum+10 and pageCount > 10}">
													<li><a href="suggestion.do?pageNum=${pageCount}">Next</a></li>
												</c:if>
												<c:if test="${!empty sessionScope.userid}">
													<li>&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-lg" onclick="location.href='suggestion_write.do?pageNum=${pageNum}'">
														글쓰기
													</button></li>
												</c:if>
												<c:if test="${empty sessionScope.userid}">
													<li>&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-lg" onclick="alert('로그인후 글 작성이 가능합니다.')">
														글쓰기
													</button></li>
												</c:if>
											</ul>
