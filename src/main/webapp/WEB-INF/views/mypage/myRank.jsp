	...
	<tr>
	 ...
	 <table class="table">
		...
		<table class="table">
	 	<thead >
		<tr>
			<th colspan="3"><font size="4"><b>내 동아리 랭킹</b></font></th>
		</tr>
		<tr>
			<th>순위</th>
			<th>점수</th>
		...
		<tr>
		<c:choose>
			<c:when test="${!empty my_club}">
				<td>${my_club.rank}</td>
				<td>${my_club.score}</td>
				<td>${my_club.club_name}</td>
			</c:when>
			<c:otherwise>
			<td colspan="3"><font><b>가입된 동아리가 없습니다.</b></font></td>
			</c:otherwise>
		</c:choose>
		</tr>
		<tr>
		<td colspan="3" align="center">
				<button type="button" class="btn btn-default" onclick="location.href='rank.do'">전체랭킹 보러가기</button>
		</td>
		</tr>
		</tbody>
		</table>
