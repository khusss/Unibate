	...
	<section id="inner-headline">
			<div class="container">
				<div class="row">
					<div class="col-lg-12">
						<ul class="breadcrumb">
							<li><a href="f_board.do?pageNum=${pageNum}"><i class="fa fa-home"></i></a><i
							...
							<div class="container">
								...
								<div class="post-image">
									<div class="post-heading">
										<h3>
											<a href="#">${fboard.fboard_subject}</a>
										</h3>
										작성자 : ${fboard.id}
									</div>
								</div>
								<br>
								<p>
									<c:if test="${!empty fboard.urldata }">
										<iframe width="560" height="315"
											src="https://www.youtube.com/embed/${fboard.urldata}"
											frameborder="0" allowfullscreen> </iframe>
									</c:if>
									<br> ${fboard.fboard_content}
								</p>
								<div class="bottom-article">
									<ul class="meta-post">
										<li><i class="icon-calendar"></i><a href="#">${fboard.fboard_date}</a></li>
										<li><i class="icon-comments"></i><a href="#">조회수
												${fboard.fboard_hit}</a></li>
										<li><i class="icon-comments"></i><a href="#">댓글수
												${reply_count}</a></li>
										<c:if test="${fboard.id eq sessionScope.userid}">
											<li><i class="icon-comments"></i><a
												href="deleteFboard.do?pageNum=${pageNum}&fboard_num=${fboard_num}">글삭제
											</a></li>
										</c:if>
									</ul>
								...
								<div class="input-group">
											...
											<c:when test="${empty sessionScope.userid}">
												<button type="button" class="btn btn-primary"
													...
													<span class="glyphicon glyphicon-thumbs-up"
														aria-hidden="true"></span> 공감 ${fboard.f_like}
												</button>
									</div>
									<div class="input-group-btn">
										<button type="button" class="btn btn-warning"
											...
											<span class="glyphicon glyphicon-thumbs-down"
												aria-hidden="true"></span> 비공감 ${fboard.f_dislike}
										</button>
										</c:when>
										<c:when test="${!empty sessionScope.userid and result != 1}">
											<button type="button" class="btn btn-primary"
												onclick="location.href='like.do?val=like&pageNum=${pageNum}&fboard_num=${fboard_num}'">
												<span class="glyphicon glyphicon-thumbs-up"
													aria-hidden="true"></span> 공감 ${fboard.f_like}
											</button>
									</div>
									<div class="input-group-btn">
										...
										<button type="button" class="btn btn-warning"
											onclick="location.href='like.do?val=dlike&pageNum=${pageNum}&fboard_num=${fboard_num}'">
											<span class="glyphicon glyphicon-thumbs-down"
												aria-hidden="true"></span> 비공감 ${fboard.f_dislike}
										</button>
										</c:when>
								...
								<div>
									<form action="f_reply_insert.do" method="get" role="form"
										class="contactForm">
										<div class="form-group">
											<textarea class="form-control" name="f_reply" rows="5"
												data-rule="required"
												data-msg="Please write something for us"
												placeholder="Message"></textarea>
											<div class="validation"></div>
											<input type="hidden" value="${pageNum}" name="pageNum">
											<input type="hidden" name="fboard_num" value="${fboard_num}">
										</div>
												...
												<table class="table">
													<tbody>
														<c:forEach items="${reply}" var="x" varStatus="status" begin="0" step="1" end="${rlen}">
															<tr>
																<c:if test="${replychk[status.index]==0}">
																	<td colspan="6"><div class="media">
																			<div class="media-left media-middle">
																				<a href="#"> <img class="media-object"
																					src="img/default_img.PNG" width="10%" height="20%">
																				</a> <br>
																			</div>
															...
															<tr>
																<c:if test="${!empty sessionScope.userid}">
																	<td width="800px">${x.f_reply}</td>
																	<c:if test="${sessionScope.userid ne x.id}">
																	<td><button class="btn btn-success"
																			onclick="location.href='r_like.do?pageNum=${pageNum}&fboard_num=${fboard_num}&f_reply_num=${x.f_reply_num}&val=like'">
																			<div class="glyphicon glyphicon-thumbs-up" />
																		</button></td>
																	<td><span class="badge">${x.f_like}</span></td>
																	<td><button class="btn btn-warning"
																			onclick="location.href='r_like.do?pageNum=${pageNum}&fboard_num=${fboard_num}&f_reply_num=${x.f_reply_num}&val=dlike'">
																			<div class="glyphicon glyphicon-thumbs-down" />
																		</button></td>
																	<td><span class="badge">${x.f_dislike}</span></td>
																	</c:if>
																	<c:if test="${sessionScope.userid eq x.id}">
																	<td><button class="btn btn-success"
																			onclick="alert('본인 댓글에는 공감 할 수 없습니다.')">
																			<div class="glyphicon glyphicon-thumbs-up" />
																		</button></td>
																	<td><span class="badge">${x.f_like}</span></td>
																	<td><button class="btn btn-warning"
																...
																<fmt:parseDate var="regDate" value="${x.f_reply_date}" pattern="yyyy-MM-dd HH:mm:ss" />
