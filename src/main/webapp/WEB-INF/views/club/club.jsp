			...
			<div class="container">
				<div class="row">
					<div class="col-lg-12">
						<div class="big-cta">
							<div class="cta-text">
								<h2><span><font size="20">동아리</font></span> 현황</h2>
							</div>
						</div>
						<div class="clearfix">
						</div>
									...
									<div class="container">
										<table class="table">
											<thead class="thead-inverse">
												<tr>
													...
													<th width="5%">인원수</th>
													<th width="15%">개설날짜</th>
													<th width="15%">가입가능여부</th>
												</tr>
											</thead>
											<tbody>
											<c:forEach items="${cboard}" var="cb">
												<tr>
													<td>${cb.club_name}</td>
													<th>${cb.club_mem} / 50</th>
													<td>${cb.club_make_date}</td>
													<c:if test="${cb.club_mem < 50}">
													<form action="clubjoin.do"  method="post">
														<td> <button id="joinBtn" class="btn btn-default">가입</button>
														<input type="hidden" name="club_name" value="${cb.club_name}">
														</td>
													</form>
													</c:if>
													<c:if test="${cb.club_mem > 50}">
										...
										<div class="text-center">
											<ul class="pagination pagination-large">
												...
												<c:if test="${pageNum > 10}">
													<li><a href="clubstatus.do?pageNum=${pageNum-10}">Prev</a></li>
												</c:if>
												<c:if test="${pageBlock*10<pageCount}">
													<c:forEach var="i" begin="${pageStart}" step="1" end="${pageEnd}">
														<li><a href="clubstatus.do?pageNum=${i}">${i}</a></li>
													</c:forEach>
												</c:if>
												<c:if test="${pageBlock*10>=pageCount}">
												...
												<c:if test="${pageCount >= pageNum+10 and pageCount > 10}">
													<li><a href="clubstatus.do?pageNum=${pageNum+10}">Next</a></li>
												</c:if>
												<c:if test="${pageCount < pageNum+10 and pageCount > 10}">
													<li><a href="clubstatus.do?pageNum=${pageCount}">Next</a></li>
												</c:if>
													<li>&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-lg" onclick="location.href='qna_write.do?pageNum=${pageNum}'">
														글쓰기
													</button></li>
											</ul>
										</div>
