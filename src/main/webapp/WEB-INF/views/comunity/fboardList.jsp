...
<div id="wrapper">
	...
	<fmt:parseDate var="regDate" value="${sb.suggestion_date}" pattern="yyyy-MM-dd" />
			...
			<div class="container">
				<div class="row">
					<div class="col-lg-12">
						<div class="big-cta">
							<div class="cta-text">
								<h2><span><font size="20">자유</font></span>게시판</h2>
							</div>
						</div>
						<div class="clearfix">
						</div>
								...
								<ul id="thumbs" class="portfolio">
									<div class="container">
										<table class="table">
											...
											<tbody>
											<c:set var="number" value="${allfboard-(pageNum-1)*15}" />
											<c:forEach items="${fboard}" var="fb">
												<tr>
													<td>${number}</td>
													<th scope="row"><a href="f_boardView.do?pageNum=${pageNum}&fboard_num=${fb.fboard_num}">${fb.fboard_subject }</a></th>
													<td>${fb.id}</td>
													<td>${fb.fboard_date}</td>
													<td>${fb.fboard_hit}</td>
												</tr>
										...
										<div class="text-center">
											<ul class="pagination pagination-large">
												...
												<c:if test="${pageNum > 10}">
													<li><a href="f_board.do?pageNum=${pageNum-10}">Prev</a></li>
												</c:if>
												<c:if test="${pageBlock*10<pageCount}">
													<c:forEach var="i" begin="${pageStart}" step="1" end="${pageEnd}">
														<li><a href="f_board.do?pageNum=${i}">${i}</a></li>
													</c:forEach>
												</c:if>
												<c:if test="${pageBlock*10>=pageCount}">
												...
												<c:if test="${pageCount >= pageNum+10 and pageCount > 10}">
													<li><a href="f_board.do?pageNum=${pageNum+10}">Next</a></li>
												</c:if>
												<c:if test="${pageCount < pageNum+10 and pageCount > 10}">
													<li><a href="f_board.do?pageNum=${pageCount}">Next</a></li>
												</c:if>
													<li>&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-lg" onclick="location.href='f_board_write.do?pageNum=${pageNum}'">
														글쓰기
													</button></li>
											</ul>
										</div>
